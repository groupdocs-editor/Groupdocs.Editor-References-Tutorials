...
- **Gestion robuste des ressources** – get images, polices et CSS en un seul appel.
- **Performance évolutive** – idéale pour le traitement par lots et la génération de rapports à grande échelle.

## Prérequis
- Java Development Kit (JDK) 8ou plus récent.
...
3. **Configurer la licence** – placez votre fichier `GroupDocs.Editor.lic` dans le dossier ressources ou défini‑la programmatique.
...
### Création et modification de documents modifiables
...
- **`Editor`** – gérer les entrées/sorties de fichiers et la détection du format.
- **`EditableDocument`** – représente le document sous forme HTML éditable.

#### Comment modifier le contenu de Word (comment modifier un mot)
Vous pouvez maintenant manipuler la chaîne HTML, remplacer les espaces réservés ou mettre à jour les styles. Après les modifications, appelez `save()` pour les enregistreurs.

### Extraction des ressources documentaires

#### Aperçu
GroupDocs.Editor facilite l'extraction des ressources intégrées telles que les images, les politiques et les fichiers CSS.
...
### Ajustement des liens externes dans le balisage HTML
...
### Enregistrement du document modifiable sur le disque
...
- **`save()`** – enregistre le HTML modifié et toutes les ressources liées au dossier spécifié.

### Vérification de l'état de mise au rebut du document modifiable
...
### Création d'un document modifiable à partir de HTML
